const VERSION='mis-2027-offline-v10';
const STATIC_CACHE=`${VERSION}-static`;
const DATA_CACHE=`${VERSION}-data`;
const STUDY_CACHE=`${VERSION}-study`;
const STATIC=['/','/index.html','/styles.css?v=20260924e-student-hub-v5','/app.js?v=20260924e-student-hub-v5','/manifest.webmanifest?v=20260924e-student-hub-v5','/favicon.svg?v=20260924e-student-hub-v5','/mis-logo.png','/icon-192.png','/icon-512.png'];
...
self.addEventListener('fetch',event=>{const req=event.request;if(req.method!=='GET')return;const url=new URL(req.url);if(url.origin!==self.location.origin)return;if(url.pathname.startsWith('/api/admin/')||url.pathname.startsWith('/api/auth/'))return;
  ...
  if(/^\/api\/(files|explanation-files|explanation-preview)\//.test(url.pathname)){event.respondWith(cacheFirst(req,STUDY_CACHE));return}
